...
**全局排除路径**：`target/、build/、out/、bin/、src/test/、src/it/、**/test/**、**/tests/**、.git/、.svn/、.idea/、.vscode/、.settings/、node_modules/、dist/、bower_components/、generated-sources/、generated-test-sources/、**/META-INF/、.gradle/、.hvigor/、.cxx/、.preview/、oh_modules/、ohosTest/、androidTest/`（仅排除非 `src/main/resources/META-INF` 的副本）
...
- 构建配置：`pom.xml`、`build.gradle`、`build.gradle.kts`、`build.xml`、`package.json`、`settings.gradle`、`settings.gradle.kts`、`build-profile.json5`、`oh-package.json5`、`hvigorfile.ts`、`.xcodeproj`、`.xcworkspace`、`Package.swift`、`Podfile`、`pubspec.yaml`、`project.config.json`（仅项目根及一级子模块）
...
- 移动端源码：Android `src/main/java/`、`src/main/kotlin/`、`src/main/cpp/`、`src/main/jni/`、`res/`、`assets/`、`AndroidManifest.xml`；鸿蒙 `src/main/ets/`、`src/main/cpp/`、`resources/`、`module.json5`；iOS 工程源码目录、`Info.plist`、`Assets.xcassets`、Storyboard/XIB
- 跨端/小程序：Flutter `lib/`；小程序 `pages/`、`components/` 与对应配置目录
...
| 跨端-ReactNative | `package.json` 含 `react-native`，或 RN 工程结构 | 不走本注册表，由 SKILL.md 前端路径处理 |
| 小程序 | `project.config.json` / `app.json`+pages，或 `taro`/`@dcloudio/uni-app` 等 | 不走本注册表，由 SKILL.md 前端路径处理 |
...
### 1.2 backend-database/ 产出决策树（数据源优先级）
...
**大型项目**（文件总数 > 200 或模块数 > 20）：scan-plan 按模块分组执行；`backend-interface` 须按 [backend-interface.md §2.3](backend/project-spec/backend-interface.md#23-分册模式大型项目拆分) 启用分册模式（入口条目 > 300 或模块数 > 20 时强制拆分）；`backend-database` 须按 [backend-database.md §2.3](backend/project-spec/backend-database.md#23-分册模式大型项目拆分) 启用分册模式（表总数 > 50 或模块数 > 20 时强制拆分）；ER 图按业务域拆分（每域 ≤ 30 表）；语义补充委托子代理，每个子代理处理一个模块。
...
3. **判定 backend-interface 输出模式**（§2.3）：入口条目 > 300 或模块数 > 20 → 分册模式；否则单文件模式。
4. **判定 backend-database 输出模式**（§2.3）：表总数 > 50 或模块数 > 20 → 分册模式；否则单文件模式。
5. 写入：
   - **backend-interface** 单文件：`fsWrite`/`fsAppend` 至 `backend-interface/index.md`（每段 ≤ 300 行）。
   - **backend-interface** 分册：创建 `backend-interface/` → `index.md`（统计 + 索引）→ 逐模块 `{module}.md`（每段 ≤ 300 行）；索引链接 `./{module}.md`。
   - **backend-database** 单文件：`fsWrite`/`fsAppend` 至 `backend-database/index.md`（每段 ≤ 300 行）。
   - **backend-database** 分册：创建 `backend-database/` → `index.md`（统计 + 索引 + ER 摘要）→ 逐模块 `{module}.md`（每段 ≤ 300 行）→ `er-diagram.md` / `dict-tables.md`（若存在）；索引链接 `./{module}.md`。
6. 写入后门禁：文档条目数 vs 中间文件条目数偏差 > 5% → 阻断；空说明 > 0 → 回头补全；分册模式下各 `index.md` 不得含 Controller 级接口表格或 `#### 表N:` 级字段表格；父目录不得存在 `backend-interface.md` 或 `backend-database.md`。
