---
name: sql-impact-analyzer
description: |
  数据变更影响分析技能。针对生产数据订正 SQL，自动完成表结构识别、代码链路追踪、后置功能关联、风险评估，
  输出结构化影响分析报告（HTML）。通过分步执行与 subagent 质量校验确保分析完整性和事实准确性。
  适用场景：当用户提供一条或多条生产数据订正 SQL，需要分析下游影响和风险时使用。
  触发词：数订正分析、数据变更影响、数据订正影响分析、SQL 影响分析、生产数据修改影响、data change impact analysis。
---

# 数据变更影响分析

## 何时使用

- 用户提供一条或多条生产数据订正 SQL，需要分析下游影响和风险
- 需要结合代码仓库定位事实源（Mapper XML、Service、EOD Job）
- 需要输出可交付的 HTML 格式影响分析报告

## 核心能力

- SQL 解析与业务语义识别
- 多仓库代码链路追踪（Entity → Mapper XML → Service → EOD Job）
- 后置功能影响分析（触发条件 vs 数据读取）
- 风险评估与分级（P0/P1/P2）
- 自检校验（通过 subagent 交叉验证）
- 结构化 HTML 报告生成

## 执行原则

- **事实源优先（强制）**：所有结论必须有代码引用支撑，禁止凭经验臆断。引用格式：`文件路径:行号`
- **区分触发 vs 读取**：WHERE/ON 条件中的引用 = 触发影响；SELECT 中的引用 = 展示影响
- **EOD 覆盖风险必查**：任何手动修改的金额/日期字段，必须检查 EOD Job 是否会重新计算覆盖
- **多仓库全覆盖**：工作区内所有代码仓库都要搜索，不得遗漏
- **未找到引用时**：明确标注「未找到代码引用，建议人工确认」
- **搜索策略**：对每个目标表名和字段名，分别使用精确匹配和模糊匹配（驼峰命名、下划线命名）进行搜索

## 规范来源

| 文档 | 用途 |
|------|------|
| `references/report-template.html` | HTML 报告模板（CSS + 结构） |

## 路径与产出约定

- **报告输出位置**：工作区根目录
- **报告命名规则**：`{场景}_data_impact_analysis.html`
  - 场景名从用户提供的上下文或 SQL 涉及的业务场景中提取
  - 示例：`还款计划调整_data_impact_analysis.html`、`利息订正_data_impact_analysis.html`
  - 无法判断场景时使用 `general_data_impact_analysis.html`

## 执行入口

按以下步骤顺序执行：

---

## Phases

### Phase: analysis

- Step 1: steps/step-01-sql-parsing.md
- Step 2: steps/step-02-code-tracing.md
- Step 3: steps/step-03-impact-analysis.md
- Step 4: steps/step-04-risk-assessment.md

### Phase: verification

- Step 5: steps/step-05-quality-check.md

### Phase: output

- Step 6: steps/step-06-report-generation.md

---

## 交付物

- `{场景}_data_impact_analysis.html`（工作区根目录）

## Agent 职责矩阵

| 步骤 | 执行方式 | 做什么 |
|------|----------|--------|
| Step 1 SQL 解析 | inline | 解析 SQL 提取表名、字段、WHERE 条件、SET 值 |
| Step 2 代码追踪 | subagent: `context-gatherer` | 搜索代码库定位所有引用 |
| Step 3 影响分析 | inline | 基于代码引用分析后置功能影响 |
| Step 4 风险评估 | inline | 对影响进行 P0/P1/P2 分级 |
| Step 5 质量校验 | subagent: `general-task-execution` | 交叉校验完整性、事实源、一致性 |
| Step 6 报告生成 | inline | 按 HTML 模板输出报告 |
