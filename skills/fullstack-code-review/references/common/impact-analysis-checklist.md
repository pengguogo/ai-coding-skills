...
>
> **适用粒度说明**：
> - 本文件所有检查项均为**全局级**，在终审步骤 Y（跨模块检查）中统一执行。
> - 影响分析天然需要跨模块视角，不适合在小节评审中执行。

---

## IA-SCOPE：改动范围分析

> 审查基线：`task/task-split.md` — 任务清单、依赖关系；`architecture-design.md` §5 — 应用内模块设计。

### IA-SCOPE-01 [I] [P1]：调用链上游影响
- **严重性**: important
- **适用粒度**: 全局级
...
### IA-SCOPE-02 [I] [P1]：调用链下游影响
- **严重性**: important
- **适用粒度**: 全局级
- **审查基线**: task/task-split.md — 依赖关系
- **判断标准**: 修改的逻辑是否影响下游消费方？MQ 消息格式变更是否通知消费方？

### IA-SCOPE-03 [B] [P1]：共享组件影响
- **严重性**: blocking
- **适用粒度**: 全局级
- **审查基线**: architecture-design.md §5 — 应用内模块设计
...
> 审查基线：`backend-design.md` §3.2 对外接口；各前端分端设计文件（`frontend-web.md` / `frontend-ios.md` / `frontend-android.md` / `frontend-hmos.md` / `frontend-h5.md`，经入口 `frontend-design.md` 定位）— 接口对齐表。

### IA-COMPAT-01 [B] [P1]：接口向后兼容
- **严重性**: blocking
- **适用粒度**: 全局级
...
## IA-MOBILE：移动端影响（仅当存在 iOS/Android/HMOS/H5 变更时执行）

> 审查基线：移动端分端设计文件 `frontend-ios.md` / `frontend-android.md` / `frontend-hmos.md`（经入口 `frontend-design.md` 定位）；`backend-design.md` §3.2 对外接口。
> 移动端原生客户端（iOS、Android、鸿蒙 HMOS）已发布到用户设备且无法强制即时更新，接口与数据契约变更须额外评估对存量 App 的影响（H5 可随发布即时更新，通常不受此限）。

### IA-MOBILE-01 [B] [P1]：接口变更对存量客户端兼容
- **严重性**: blocking
- **适用粒度**: 全局级
- **审查基线**: backend-design.md §3.2 — 对外接口
- **判断标准**: 后端接口变更（字段删除/类型变更/语义变更）是否兼容**已发布的旧版本 App**？旧版 App 在新接口下是否仍能正常工作？
- **示例**:
  ```
  ✅ 接口新增字段为可选，旧版 App 忽略即可正常运行
  ❌ 删除/重命名旧版 App 仍在使用的字段，导致存量用户解析崩溃
  ```

### IA-MOBILE-02 [I] [P1]：本地存储迁移（Room / CoreData）
- **严重性**: important
- **适用粒度**: 全局级
- **审查基线**: frontend-android.md / frontend-ios.md / frontend-hmos.md — 数据层
- **判断标准**: Android Room / iOS CoreData / HMOS RelationalStore 的 schema 变更是否提供 Migration？覆盖安装升级后存量本地数据是否不丢失、不崩溃？

### IA-MOBILE-03 [I] [P1]：最低版本与 API 兼容
- **严重性**: important
- **适用粒度**: 全局级
- **审查基线**: frontend-android.md / frontend-ios.md / frontend-hmos.md — 最低版本与兼容
- **判断标准**: 使用的新 API 是否有版本判断或兼容封装（Android `minSdk` 判断 / iOS `@available`、`if #available` / HMOS `canIUse`）？是否影响低版本设备运行？

### IA-MOBILE-04 [S] [P2]：强制更新与灰度
- **严重性**: suggestion
- **适用粒度**: 全局级
- **审查基线**: frontend-android.md / frontend-ios.md / frontend-hmos.md — 兼容策略
- **判断标准**: 不兼容变更是否有客户端版本检测与强制/建议更新机制？是否支持按版本灰度下发？
