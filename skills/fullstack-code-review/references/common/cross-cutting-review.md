...
>
> **适用粒度说明**：
> - **模块级**：在按 task-split.md 主分类/功能点分组的模块审查中执行。
> - **全局级**：在全局设计合规检查（步骤 2）或全局影响分析（步骤 5）中执行。
> - **前端审查阶段**：在步骤 4 前端审查中执行（此时后端审查已完成，可作为对比基线）。

---

## CC-SEC：安全审查

### CC-SEC-01 [B] [P0]：敏感信息不硬编码
- **严重性**: blocking
- **适用粒度**: 模块级
...
### CC-PERF-04 [S] [P2]：大文件/大数据量处理
- **严重性**: suggestion
- **适用粒度**: 模块级
- **规范来源**: 通用性能规范
- **判断标准**: 文件导出使用流式写入；大批量数据处理使用分批处理；前端大文件上传使用分片

### CC-PERF-05 [S] [P2]：缓存使用合理
- **严重性**: suggestion
- **适用粒度**: 模块级
- **规范来源**: 通用性能规范
...
### CC-ARCH-01 [I] [P1]：新增文件符合模块边界
- **严重性**: important
- **适用粒度**: 模块级
- **规范来源**: backend-design.md / 前端分端设计文件（`frontend-web.md` 等，经 `frontend-design.md` 入口定位）
...
### CC-ARCH-03 [S] [P2]：公共代码提取合理
- **严重性**: suggestion
- **适用粒度**: 模块级
- **规范来源**: 通用架构规范
- **判断标准**: 重复代码（≥3 处相同逻辑）须提取为公共方法/组件/工具类

### CC-ARCH-04 [B] [P0]：接口契约前后端一致
- **严重性**: blocking
- **适用粒度**: 前端审查阶段
- **规范来源**: backend-design.md + 前端分端设计文件（`frontend-web.md` 等，经 `frontend-design.md` 入口定位）
...
> **执行时机说明**：此检查项在 Step 4（前端功能点级审查）中执行，此时后端审查已完成（或并行完成），后端实现可作为对比基线。

### CC-ARCH-05 [I] [P1]：错误码前后端统一
- **严重性**: important
- **适用粒度**: 前端审查阶段
- **规范来源**: backend-design.md
- **判断标准**: 后端返回的错误码须在前端有对应的处理逻辑和用户友好提示

> **执行时机说明**：此检查项在 Step 4（前端功能点级审查）中执行，此时后端审查已完成（或并行完成），后端错误码定义可作为对比基线。

---

## CC-DESIGN：设计合规

### CC-DESIGN-01 [I] [P0]：实现与设计文档一致
- **严重性**: important
- **适用粒度**: 全局级
- **规范来源**: backend-design.md / 前端分端设计文件（`frontend-web.md` 等，经 `frontend-design.md` 入口定位）
- **判断标准**: 代码实现的类结构、方法签名、业务流程须与设计文档描述一致；偏差须有合理说明

> **执行时机说明**：此检查项在 Step 2（全局设计合规检查）中执行，需要全量对比设计文档与代码变更。

### CC-DESIGN-02 [I] [P0]：领域模型字段完整
- **严重性**: important
- **适用粒度**: 全局级
- **规范来源**: backend-design.md
...
> **执行时机说明**：此检查项在 Step 2（全局设计合规检查）中执行，需要对比整个设计文档的领域模型定义。

### CC-DESIGN-03 [S] [P2]：异常处理与设计约定一致
- **严重性**: suggestion
- **适用粒度**: 模块级
- **规范来源**: backend-design.md
- **判断标准**: 错误码定义、异常分类、异常处理策略须与设计文档中的约定一致

### CC-DESIGN-04 [I] [P0]：时序逻辑与设计吻合
- **严重性**: important
- **适用粒度**: 模块级
- **规范来源**: backend-design.md
...
### CC-AI-02 [B] [P1]：AI 生成代码无安全隐患
- **严重性**: blocking
- **适用粒度**: 模块级
- **规范来源**: 通用安全规范
- **判断标准**: AI 生成的代码须重点检查：SQL 注入、XSS、硬编码密钥、不安全的反序列化、过于宽泛的权限

### CC-AI-03 [I] [P1]：AI 生成代码逻辑正确性
- **严重性**: important
- **适用粒度**: 模块级
- **规范来源**: 通用质量规范
