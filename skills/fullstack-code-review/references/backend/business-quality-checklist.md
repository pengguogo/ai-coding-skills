...
>
> **适用粒度说明**：
> - **模块级**：在按 task-split.md 主分类分组的模块审查中执行。
> - **全局级**：在全局影响分析（步骤 5）中执行，或在模块审查中标记 `[需全局确认]` 后在步骤 5 复核。
...
### BQ-IDEM-03 [I] [P1]：重复调用安全
- **严重性**: important
- **适用粒度**: 模块级
- **审查基线**: backend-design.md §3.2 — 幂等
- **判断标准**: 同一请求重复提交不产生重复数据、不重复扣款、不重复发送通知

### BQ-IDEM-04 [S] [P2]：幂等窗口合理
- **严重性**: suggestion
- **适用粒度**: 模块级
- **审查基线**: backend-design.md §3.2 — 幂等
...
### BQ-TX-03 [B] [P0]：rollbackFor 显式声明
- **严重性**: blocking
- **适用粒度**: 模块级
- **审查基线**: 通用
...
### BQ-CONCUR-02 [I] [P1]：锁粒度合理
- **严重性**: important
- **适用粒度**: 模块级
- **审查基线**: backend-design.md §3.2
- **判断标准**: 锁的粒度与业务匹配（行级锁优于表级锁），锁持有时间尽量短

### BQ-CONCUR-03 [I] [P1]：分布式锁使用正确
- **严重性**: important
- **适用粒度**: 模块级
- **审查基线**: backend-design.md §2.2
...
### BQ-EXCEPT-02 [I] [P1]：部分失败处理
- **严重性**: important
- **适用粒度**: 模块级
- **审查基线**: backend-design.md §3.2 — 异常处理
- **判断标准**: 批量操作中部分失败时的处理策略明确（全部回滚 / 跳过失败项继续 / 记录失败项后续补偿）

### BQ-EXCEPT-03 [I] [P1]：异常信息完整
- **严重性**: important
- **适用粒度**: 模块级
- **审查基线**: 通用
...
## BQ-SPRING：Spring 框架陷阱

> 审查基线：通用。

### BQ-SPRING-01 [B] [P0]：事务 self-invocation 防护
- **严重性**: blocking
- **适用粒度**: 模块级
- **审查基线**: 通用
- **判断标准**: 同一个类中方法 A 直接调用方法 B（`this.methodB()` 或无限定符的 `methodB()`），若 B 上有 @Transactional 注解，则事务不生效。须通过注入自身代理（`@Lazy` self-injection）或拆分到不同类来解决。同理适用于 @Async、@Cacheable 等基于代理的注解。
- **示例**:
  ```java
  ❌ @Service
  public class OrderService {
      public void createOrder(OrderDTO dto) {
          this.saveOrderLog(dto); // saveOrderLog 的 @Transactional 不生效！
      }
      @Transactional(propagation = Propagation.REQUIRES_NEW)
      public void saveOrderLog(OrderDTO dto) { }
  }
  ...
  public void createOrder(OrderDTO dto) {
      self.saveOrderLog(dto); // 通过代理调用，事务生效
  }
  ```
...
### BQ-RES-01 [B] [P1]：流/连接资源必须关闭
- **严重性**: blocking
- **适用粒度**: 模块级
- **审查基线**: 通用
- **判断标准**: InputStream/OutputStream/Connection/ResultSet 等实现 AutoCloseable 的资源必须在 try-with-resources 或 finally 块中关闭；不依赖 GC 回收。
- **示例**:
  ```java
...
> 审查基线：`backend-design.md` §3.2 — MQ 消费设计。

### BQ-MQ-01 [B] [P1]：MQ 消费端幂等
- **严重性**: blocking
- **适用粒度**: 模块级
- **审查基线**: backend-design.md §3.2 — MQ 消费设计
- **判断标准**: MQ 消费者必须实现幂等（基于消息 ID 或业务键去重），因为消息可能重复投递（broker 重试、网络抖动）。
- **示例**:
  ```java
  ✅ public void onMessage(OrderMessage msg) {
      if (consumeRecordMapper.exists(msg.getMsgId())) {
          log.info("消息已消费, msgId={}", msg.getMsgId());
          return;
      }
      // 业务处理 + 记录消费记录（同事务）
  }
  ...
  ❌ public void onMessage(OrderMessage msg) {
      orderService.process(msg); // 无幂等校验
  }
  ```
