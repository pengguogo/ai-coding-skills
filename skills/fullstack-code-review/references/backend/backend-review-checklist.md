...
> 每个检查项标注严重性、规范来源、判断标准和**适用粒度**。blocking / important 级别检查项附带正反示例。
...
> - **全局级**：在全局设计合规检查（步骤 2）或全局影响分析（步骤 5）中执行。

---

## BE-NAME：命名规范（§2）

### BE-NAME-01 [I] [P1]：类名使用 UpperCamelCase
- **严重性**: important
- **适用粒度**: 模块级
...
### BE-NAME-02 [I] [P1]：方法名使用 lowerCamelCase
- **严重性**: important
- **适用粒度**: 模块级
...
### BE-NAME-03 [I] [P1]：常量名使用 UPPER_SNAKE_CASE
- **严重性**: important
- **适用粒度**: 模块级
...
### BE-ARCH-02 [I] [P1]：DTO/VO/Entity 职责分离
- **严重性**: important
- **适用粒度**: 模块级
...
### BE-PATH-01 [B] [P0]：API 路径与设计文档一致
- **严重性**: blocking
- **适用粒度**: 模块级
- **规范来源**: coding-standard.md §3.3 + backend-design.md
- **判断标准**: `@RequestMapping` 路径须与 `backend-design.md` 中接口定义完全一致；路径使用小写字母和连字符，名词复数形式
- **示例**:
  ```java
...
### BE-DOC-01 [I] [P1]：公共类和方法必须有 Javadoc
- **严重性**: important
- **适用粒度**: 模块级
...
### BE-AI-01 [I] [P1]：非 Java 文件的 AI 标记
- **严重性**: important
- **适用粒度**: 模块级
...
### BE-MVC-01 [I] [P1]：Controller 方法使用明确的 HTTP 方法注解
- **严重性**: important
- **适用粒度**: 模块级
...
### BE-DESIGN-01 [I] [P1]：异常使用自定义业务异常类
- **严重性**: important
- **适用粒度**: 模块级
...
### BE-TEST-01 [I] [P1]：核心业务逻辑有单元测试
- **严重性**: important
- **适用粒度**: 模块级
- **规范来源**: coding-standard.md §10.1
- **判断标准**: Service 层核心方法须有对应的单元测试，覆盖正常流程和主要异常分支；测试方法名体现测试意图（推荐 `should_预期行为_when_条件` 格式）；测试数据自包含，不依赖外部环境
...
### BE-SEC-05 [I] [P1]：文件上传校验
- **严重性**: important
- **适用粒度**: 模块级
- **规范来源**: coding-standard.md §11.5
- **判断标准**: 文件上传须校验文件类型、大小限制，存储路径不可由用户控制

### BE-SEC-06 [B] [P0]：JSON 反序列化安全
- **严重性**: blocking
- **适用粒度**: 模块级
- **规范来源**: 通用安全规范
- **判断标准**: Fastjson 禁止开启 autoType（远程代码执行风险）；Jackson 禁止开启 defaultTyping（反序列化漏洞）；不反序列化不可信来源的数据
- **示例**:
  ```java
