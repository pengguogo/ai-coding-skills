...
- input: Step 1-7 的产出（经 Step 8 校验通过）
- output_file: `{场景}_data_impact_analysis.html`（工作区根目录）
- reference: `references/report-template.html`

## 执行指令

将 Step 1-7 的结果按 `references/report-template.html` 的 HTML 模板结构组装为最终报告。
...
---分隔线---
二、订正字段合理性分析
  2.1 字段类型校验（field-check-table）
  2.2 合理值范围检查
  2.3 字段依赖一致性
  2.4 WHERE 覆盖风险
---分隔线---
三、ER 模型关联影响
  3.1 数据模型全景（er-diagram）
  3.2 关联表清单 — {ER_RELATION_ROWS}
  3.3 字段影响详情 — {ER_DETAIL_ROWS}
---分隔线---
四、重复数据影响分析
  4.1 订正 SQL 重复风险 — {DUP_SQL_ROWS}
  4.2 关联表重复风险 — {DUP_RELATED_ROWS}
  4.3 验证脚本 — check_before / check_after / rollback
---分隔线---
五、逐条影响分析
  SQL-1：{表名}（{中文含义}）
    - 字段含义（field-table）
...
| 占位符 | 来源 | 格式 | 说明 |
|--------|------|------|------|
| `{FIELD_TYPE_ROWS}` | Step 3 | HTML `<tr>` | 字段类型校验明细行 |
| `{FIELD_RANGE_ROWS}` | Step 3 | HTML `<tr>` | 合理值范围检查明细行 |
| `{FIELD_DEP_ROWS}` | Step 3 | HTML `<tr>` | 字段依赖一致性明细行 |
| `{WHERE_RISK_ROWS}` | Step 3 | HTML `<tr>` | WHERE覆盖风险明细行 |
| `{ER_DIAGRAM}` | Step 4 | HTML 块 | ER模型全景（er-diagram div） |
| `{ER_RELATION_ROWS}` | Step 4 | HTML `<tr>` | 关联表清单明细行 |
| `{ER_DETAIL_ROWS}` | Step 4 | HTML `<tr>` | 字段影响详情明细行 |
| `{DUP_SQL_ROWS}` | Step 5 | HTML `<tr>` | 订正SQL重复风险明细行 |
| `{DUP_RELATED_ROWS}` | Step 5 | HTML `<tr>` | 关联表重复风险明细行 |
| `{CHECK_BEFORE_SQL}` | Step 5 | SQL 文本 | 订正前检查 SQL（pre-formatted） |
| `{CHECK_AFTER_SQL}` | Step 5 | SQL 文本 | 订正后验证 SQL |
| `{ROLLBACK_SQL}` | Step 5 | SQL 文本 | 回滚 SQL |
